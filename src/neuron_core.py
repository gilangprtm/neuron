"""Neuron Graph Core — standalone neural activation layer.

Node: memory | vault_note | project | session
Edge: co_activated (weight, count, last_seen)
Operations: activate / strengthen / decay / add_node / delete_node / status

Storage: SQLite (data/graph.db) with JSON fallback.
Embeddings: FastEmbed (BAAI/bge-small-en-v1.5, 384-dim) via neuron_embed.
TF-IDF: scikit-learn cosine similarity for token-based fallback.
"""
